...
class Action
{
	...
	function login()
	{
		...
		$qry = $this->db->query("SELECT *,concat(firstname,' ',lastname) as name FROM {$type[$login]} where username = '" . $username . "' and password = '" . md5($password) . "'  ");
		if ($qry->num_rows > 0) {
	...
	function save_faculty()
	{
		...
		$check = $this->db->query("SELECT * FROM faculty_list where username ='$username' " . (!empty($id) ? " and id != {$id} " : ''))->num_rows;
		if ($check > 0) {
			return 2;
			exit;
		}
	...
	function save_student()
	{
		...
		$check = $this->db->query("SELECT * FROM student_list where username ='$username' " . (!empty($id) ? " and id != {$id} " : ''))->num_rows;
		if ($check > 0) {
			return 2;
			exit;
		}
	...
	function get_report()
	{
		...
		// Get feedback data
		$feedback_query = $this->db->query("SELECT af.rating, af.comment, CONCAT(sl.firstname, ' ', sl.lastname) as student_name 
