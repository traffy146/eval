			...
			<div class="list-group">
				...
				while ($row = $restriction->fetch_array()):
					...
					<a class="list-group-item list-group-item-action <?php echo isset($rid) && $rid == $row['id'] ? 'active' : '' ?>"
						href="./index.php?page=evaluate&rid=<?php echo $row['id'] ?>&sid=<?php echo $row['sid'] ?>&fid=<?php echo $row['fid'] ?>"><?php echo ucwords($row['faculty']) . ' - (' . $row["code"] . ') ' . $row['subject'] ?></a>
				<?php endwhile; ?>
			</div>
		</div>
		<div class="col-md-9">
...
<?php endfor; ?>
    ...
    <div class="form-group mb-3">
        ...
        <div class="icheck-primary d-inline mr-3">
            <input type="radio" id="rating_neutral" name="rating" value="Neutral">
            <label for="rating_neutral">Neutral</label>
        </div>
